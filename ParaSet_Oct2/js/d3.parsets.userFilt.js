...
function build_parallel_sets(curves_checkbox){
  ...
  d3.csv("data/GOT_battles.csv", function(error, csv) {
    ...
    var resultArray2 = limitToSomeNew(csv, userFilt["limitations"]);
...
$(document).ready(function(){
  $("input#curved").click(function(){
    $("div#vis").empty();
    var checked=true; 
    // d3.parsets.tension(0.4);
    // build_parallel_sets(checked)
    // vis.curves(this);
    vis = build_parallel_sets(checked)
    vis.curves.call(this)
    console.log("got to end of input#curved click function")
    // vis = build_parallel_sets(checked).curves.call(this);
    // bigViz.curves(this);
    // bigViz = build_parallel_sets().curves.call(this);
  })
})
